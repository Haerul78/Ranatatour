...
export default function Edit({ article }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        label: article.label ?? "",
        is_featured: article.is_featured ?? false, sort_order: article.sort_order ?? 0,
    });
    ...
    return (
        <AdminLayout title="Edit Artikel" subtitle={`Mengedit: ${article.title}`}>
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
                    <div className="space-y-5">
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {["Tips", "Cerita", "Kuliner", "Panduan", "Budaya"].map(l => (
                                                <button key={l} type="button"
                                                    ...
                                                    className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${data.label === l ? "border-[#991612] bg-[#fdecea] text-[#991612]" : "border-[#e8d5ce] text-[#9b8f8a] hover:border-[#991612] hover:text-[#991612]"}`}>{l}</button>
                                            ))}
                                        </div>
                                        <Input value={data.label} onChange={v => setData("label", v)} placeholder="Kustom..." error={errors.label} />
                                    </div>
                                </Field>
                                <Field label="Tanggal Publish" error={errors.published_at}>
                                    <Input type="date" value={data.published_at} onChange={v => setData("published_at", v)} error={errors.published_at} />
                                </Field>
                            </div>
                        </Card>
                        ...
                        <Card title="Media & Ringkasan" accent="#c76854">
                            <Field label="URL Gambar" error={errors.image_url}>
                                <Input value={data.image_url} onChange={v => setData("image_url", v)} placeholder="/images/artikel.jpg" error={errors.image_url} />
                                <ImagePreview src={data.image_url} />
                            </Field>
                            <Field label="Ringkasan" error={errors.excerpt}>
                                <textarea value={data.excerpt} onChange={e => setData("excerpt", e.target.value)}
                                    rows={2} placeholder="Deskripsi singkat yang muncul di kartu artikel..." className={textareaClass(errors.excerpt)} />
                            </Field>
                        </Card>
                        ...
                        <Card title="Isi Artikel" accent="#df7f6e">
                            <Field label="Konten" error={errors.content}>
                                <textarea value={data.content} onChange={e => setData("content", e.target.value)}
                                    rows={10} placeholder="Tulis konten artikel di sini..." className={textareaClass(errors.content)} />
                            </Field>
                        </Card>
                    </div>
                    ...
                    <div className="space-y-5">
                        <Card title="Pengaturan" accent="#e3a39f">
                            <Field label="Status">
                                <Toggle checked={data.is_featured} onChange={v => setData("is_featured", v)} label="Tampilkan sebagai Featured" desc="Artikel akan muncul di bagian unggulan" />
                            </Field>
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <><Spinner />Menyimpan...</> : <><SaveIcon />Perbarui Artikel</>}
                            </button>
                            <Link href="/admin/articles" className="flex items-center justify-center rounded-xl border border-[#e0ccc5] py-3 text-sm font-semibold text-[#7c6f6a] transition hover:border-[#991612] hover:text-[#991612]">
                                Batal
                            </Link>
                        </div>
                    </div>
