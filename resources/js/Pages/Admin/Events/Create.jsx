...
export default function Create() {
    ...
    return (
        <AdminLayout title="Tambah Event" subtitle="Lengkapi informasi event baru untuk ditampilkan di website.">
            <form onSubmit={handleSubmit}>
                <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
                    <div className="space-y-5">
                        <Card title="Informasi Utama" accent="#991612">
                            <Field label="Judul Event" required error={errors.title}>
                                <Input value={data.title} onChange={v => setData("title", v)}
                                    placeholder="Contoh: Jazz Traffic Festival" error={errors.title} autoFocus />
                            </Field>
                            <div className="grid gap-4 sm:grid-cols-3">
                                <Field label="Lokasi" error={errors.location}>
                                    <Input value={data.location} onChange={v => setData("location", v)}
                                        placeholder="Jakarta, Indonesia" error={errors.location} />
                                </Field>
                                <Field label="Tanggal" error={errors.date}>
                                    <Input value={data.date} onChange={v => setData("date", v)}
                                        placeholder="12 Mei 2026" error={errors.date} />
                                </Field>
                            ...
                            <Field label="Badge" error={errors.badge}>
                                <div className="flex flex-wrap gap-2">
                                    {["Festival", "Musik", "Budaya", "Populer"].map(b => (
                        ...
                        <Card title="Media & Konten" accent="#c76854">
                            <Field label="URL Gambar" error={errors.image_url}>
                                <Input value={data.image_url} onChange={v => setData("image_url", v)}
                                    placeholder="/images/event.jpg" error={errors.image_url} />
                                <ImagePreview src={data.image_url} />
                            </Field>
                            <Field label="Ringkasan" error={errors.excerpt}>
                                <Input value={data.excerpt} onChange={v => setData("excerpt", v)}
                                    placeholder="Deskripsi singkat yang muncul di kartu event..." error={errors.excerpt} />
                            </Field>
                            <Field label="Deskripsi Lengkap" error={errors.description}>
                                <textarea value={data.description} onChange={e => setData("description", e.target.value)}
                                    rows={5} placeholder="Detail lengkap tentang event ini..."
                    ...
                    <div className="space-y-5">
                        <Card title="Pengaturan" accent="#e3a39f">
                            <Field label="Status">
                                <Toggle checked={data.is_featured} onChange={v => setData("is_featured", v)}
                                    ...
                                    desc="Event akan muncul di bagian unggulan" />
                            </Field>
                        ...
                        <TipCard items={[
                            "Gunakan nama event yang jelas dan mudah diingat",
                            "Isi tanggal dengan format yang konsisten",
                            "Tambahkan gambar untuk meningkatkan daya tarik",
                            "Aktifkan Featured agar event tampil di halaman utama",
                        ]} />
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <><Spinner />Menyimpan...</> : <><SaveIcon />Simpan Event</>}
                            </button>
