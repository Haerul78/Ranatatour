...
export default function Edit({ event }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        date: event.date ?? "", price: event.price ?? "",
        badge: event.badge ?? "", image_url: event.image_url ?? "",
        excerpt: event.excerpt ?? "", description: event.description ?? "",
        is_featured: event.is_featured ?? false, sort_order: event.sort_order ?? 0,
    });
    ...
    return (
        <AdminLayout title="Edit Event" subtitle={`Mengedit: ${event.title}`}>
                    ...
                    <div className="space-y-5">
                        <Card title="Informasi Utama" accent="#991612">
                            ...
                            <div className="grid gap-4 sm:grid-cols-3">
                                <Field label="Lokasi" error={errors.location}>
                                    <Input value={data.location} onChange={v => setData("location", v)} placeholder="Jakarta, Indonesia" error={errors.location} />
                                </Field>
                                <Field label="Tanggal" error={errors.date}>
                                    <Input value={data.date} onChange={v => setData("date", v)} placeholder="12 Mei 2026" error={errors.date} />
                                </Field>
                        ...
                        <Card title="Media & Konten" accent="#c76854">
                            <Field label="URL Gambar" error={errors.image_url}>
                                <Input value={data.image_url} onChange={v => setData("image_url", v)} placeholder="/images/event.jpg" error={errors.image_url} />
                                <ImagePreview src={data.image_url} />
                            </Field>
                            <Field label="Ringkasan" error={errors.excerpt}>
                                <Input value={data.excerpt} onChange={v => setData("excerpt", v)} placeholder="Deskripsi singkat yang muncul di kartu event..." error={errors.excerpt} />
                            </Field>
                            <Field label="Deskripsi Lengkap" error={errors.description}>
                                <textarea value={data.description} onChange={e => setData("description", e.target.value)}
                                    rows={5} placeholder="Detail lengkap tentang event ini..." className={textareaClass(errors.description)} />
                            </Field>
                        </Card>
                    </div>
                    <div className="space-y-5">
                        <Card title="Pengaturan" accent="#e3a39f">
                            <Field label="Status">
                                <Toggle checked={data.is_featured} onChange={v => setData("is_featured", v)} label="Tampilkan sebagai Featured" desc="Event akan muncul di bagian unggulan" />
                            </Field>
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <><Spinner />Menyimpan...</> : <><SaveIcon />Perbarui Event</>}
                            </button>
                            <Link href="/admin/events" className="flex items-center justify-center rounded-xl border border-[#e0ccc5] py-3 text-sm font-semibold text-[#7c6f6a] transition hover:border-[#991612] hover:text-[#991612]">
                                Batal
                            </Link>
                        </div>
                    </div>
