...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        title: "", location: "", price: "", duration: "",
        badge: "", image_url: "", excerpt: "", description: "",
    ...
    return (
        <AdminLayout title="Tambah Tour" subtitle="Lengkapi informasi tour baru untuk ditampilkan di website.">
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
                    ...
                    <div className="space-y-5">
                        ...
                        <Card title="Informasi Utama" accent="#991612">
                            <Field label="Judul Tour" required error={errors.title}>
                                <Input value={data.title} onChange={v => setData("title", v)}
                                    placeholder="Contoh: Bali Paradise Tour" error={errors.title} autoFocus />
                            </Field>
                            <div className="grid gap-4 sm:grid-cols-3">
                                <Field label="Lokasi" error={errors.location}>
                                    <Input value={data.location} onChange={v => setData("location", v)}
                                        placeholder="Bali, Indonesia" error={errors.location} />
                                </Field>
                                <Field label="Harga" error={errors.price}>
                                    <Input value={data.price} onChange={v => setData("price", v)}
                                        placeholder="Rp 3.500.000" error={errors.price} />
                                </Field>
                                <Field label="Durasi" error={errors.duration}>
                                    <Input value={data.duration} onChange={v => setData("duration", v)}
                                        placeholder="3 Hari 2 Malam" error={errors.duration} />
                                </Field>
                            </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {["Populer", "Baru", "Hemat", "Featured"].map(b => (
                                        <button key={b} type="button"
                                            onClick={() => setData("badge", data.badge === b ? "" : b)}
                                            className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                                                data.badge === b
                                                    ? "border-[#991612] bg-[#fdecea] text-[#991612]"
                                                    : "border-[#e8d5ce] text-[#9b8f8a] hover:border-[#991612] hover:text-[#991612]"
                                            }`}>{b}</button>
                                    ))}
                                    <Input value={data.badge} onChange={v => setData("badge", v)}
                                        placeholder="Kustom..." error={errors.badge} className="w-32" />
                                </div>
                            </Field>
                        </Card>
                        ...
                        <Card title="Media & Konten" accent="#c76854">
                            <Field label="URL Gambar" error={errors.image_url}>
                                <Input value={data.image_url} onChange={v => setData("image_url", v)}
                                    placeholder="/images/bali.jpg" error={errors.image_url} />
                                <ImagePreview src={data.image_url} />
                            </Field>
                            <Field label="Ringkasan" error={errors.excerpt}>
                                <Input value={data.excerpt} onChange={v => setData("excerpt", v)}
                                    placeholder="Deskripsi singkat yang muncul di kartu tour..." error={errors.excerpt} />
                            </Field>
                            <Field label="Deskripsi Lengkap" error={errors.description}>
                                <textarea value={data.description} onChange={e => setData("description", e.target.value)}
                                    rows={5} placeholder="Ceritakan detail perjalanan, itinerary, dan keunggulan tour ini..."
                    ...
                    <div className="space-y-5">
                        <Card title="Pengaturan" accent="#e3a39f">
                            <Field label="Status">
                                <Toggle
                                    ...
                                    desc="Tour akan muncul di bagian unggulan"
                                />
                            </Field>
                            <Field label="Urutan Tampil" error={errors.sort_order}>
                                <Input type="number" value={data.sort_order} onChange={v => setData("sort_order", v)}
                                    placeholder="0" error={errors.sort_order} />
                        ...
                        <TipCard items={[
                            ...
                            "Upload gambar dengan rasio 4:3 untuk tampilan terbaik",
                            "Isi deskripsi lengkap agar calon wisatawan tertarik",
                            "Aktifkan Featured agar tour muncul di halaman utama",
                        ]} />
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button type="submit" disabled={processing}
                                ...
                                {processing
                                    ? <><Spinner />Menyimpan...</>
                                    : <><SaveIcon />Simpan Tour</>}
                            </button>
                            <Link href="/admin/tours"
                                className="flex items-center justify-center rounded-xl border border-[#e0ccc5] py-3 text-sm font-semibold text-[#7c6f6a] transition hover:border-[#991612] hover:text-[#991612]">
                                Batal
                            </Link>
                        </div>
                    </div>
    ...
    return (
        <div>
            <label className="mb-1.5 flex items-center gap-1 text-xs font-semibold text-[#4b2e2b]">
                {label}
                {required && <span className="text-[#991612]">*</span>}
            </label>
            {children}
...
function Input({ value, onChange, placeholder, error, type = "text", autoFocus, className = "" }) {
    return (
        <input type={type} value={value} onChange={e => onChange(e.target.value)}
            ...
            className={`w-full rounded-xl border bg-[#fdfaf9] px-3.5 py-2.5 text-sm text-[#2d1a1a] placeholder-[#c4aea6] outline-none transition focus:bg-white focus:ring-2 ${className} ${
...
function Toggle({ checked, onChange, label, desc }) {
    return (
        <button type="button" onClick={() => onChange(!checked)}
            className="flex w-full items-start gap-3 rounded-xl border border-[#e8d5ce] bg-[#fdfaf9] p-3.5 text-left transition hover:border-[#c4aea6]">
    ...
    return (
        <div className="rounded-2xl border border-[#e8d5ce] bg-[#fdf7f4] p-4">
            ...
            <ul className="space-y-1.5">
                {items.map((tip, i) => (
                    <li key={i} className="flex items-start gap-2 text-[11px] text-[#7c6f6a]">
                        <span className="mt-0.5 text-[#991612]">•</span>{tip}
                    </li>
                ))}
            </ul>
        </div>
