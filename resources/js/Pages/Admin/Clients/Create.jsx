...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        name: "", logo_url: "", testimonial: "", is_featured: false, sort_order: 0,
    });
    ...
    return (
        <AdminLayout title="Tambah Klien" subtitle="Tambahkan partner atau klien baru Ranata Tour.">
            <form onSubmit={handleSubmit}>
                <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
                        ...
                        <Card title="Informasi Klien" accent="#991612">
                            <Field label="Nama Klien / Perusahaan" required error={errors.name}>
                                <Input value={data.name} onChange={v => setData("name", v)}
                                    placeholder="Contoh: PT. Maju Bersama" error={errors.name} autoFocus />
                            </Field>
                            <Field label="URL Logo" error={errors.logo_url}>
                                <Input value={data.logo_url} onChange={v => setData("logo_url", v)}
                                    placeholder="/images/logo-klien.png" error={errors.logo_url} />
                    ...
                    <div className="space-y-5">
                        <Card title="Pengaturan" accent="#e3a39f">
                            <Field label="Status">
                                <Toggle checked={data.is_featured} onChange={v => setData("is_featured", v)}
                                    ...
                                    desc="Klien akan muncul di bagian unggulan" />
                            </Field>
                        ...
                        <TipCard items={[
                            "Gunakan nama resmi perusahaan klien",
                            "Logo sebaiknya berformat PNG transparan",
                            "Testimoni yang spesifik lebih meyakinkan",
                            "Aktifkan Featured agar klien tampil di halaman utama",
                        ]} />
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <><Spinner />Menyimpan...</> : <><SaveIcon />Simpan Klien</>}
                            </button>
